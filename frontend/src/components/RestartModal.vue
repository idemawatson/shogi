      ...
      <v-card>
        ...
        <div v-else class="tablespace">
          <table>
            ...
            <th>手数</th>
            <tr
              v-for="[i, s] of saves.entries()"
              :key="i"
              @click="gameId = s.game_id"
              :class="{ selected: gameId == s.game_id }"
            >
              <td>{{ i + 1 }}</td>
          ...
          <v-card-actions style="justify-content: center;">
            <v-btn color="green darken-1" text @click="restart">YES</v-btn>
            <v-btn color="green darken-1" text @click="closeModal">NO</v-btn>
          </v-card-actions>
        </div>
      </v-card>
    </v-dialog>
